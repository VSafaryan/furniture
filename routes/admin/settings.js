...
router.post("/conditionPay", async (req, res) => {
  if (req.body.id) {
    const { id } = req.body;
    delete req.body.id;
    await ConditionPay.findByIdAndUpdate(id, req.body);
  } else {
    const post = new ConditionPay({
      conditionPayHy: req.body.conditionPayHy,
      conditionPayRu: req.body.conditionPayRu,
      conditionPayEn: req.body.conditionPayEn,
    });
    await post.save();
  }
